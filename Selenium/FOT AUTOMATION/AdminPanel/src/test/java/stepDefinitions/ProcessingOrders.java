...
//		processingOrdersPage.clickEditOption();
	...
	public void user_processes_the_order_with_below_data(io.cucumber.datatable.DataTable dataTable) {
		...
		if (orderData.getActualYield() != null) {
			...
			if (orderData.getWastageProduct() != null) {
...
//		}
	...
	public void select_the_batch_details_and_verify_the_added_quantity_and_the_performed_action(String string) {
		...
		String ActualQtyStockIn = orderData.getActualYield() != null ? orderData.getActualYield()
				: orderData.getRequestedQty();
	...
	public void select_the_batch_details_and_verify_the_added_quantity_for_wastage_and_the_performed_action(
			String string) {
		stockSummaryPage.selectBatchDetails("Batch Details");
		stockSummaryPage.clickReferenceFilterIcon();
		System.out.println("Processing Order ID for Stock Verification: " + scenarioContext.getProcessingOrderId());
...
//		String chanedUom = string;
	...
	public void select_the_batch_details_and_verify_that_stock_entry_is_created_for_the_updated_requested_quantity_with_the_action(
			String string) {
		stockSummaryPage.selectBatchDetails("Batch Details");
		stockSummaryPage.clickReferenceFilterIcon();
		System.out.println("Processing Order ID for Stock Verification: " + scenarioContext.getProcessingOrderId());
	...
	@When("at draft status user changes the source product to {string}")
	public void at_draft_status_user_changes_the_source_product_to(String string) {
		scenarioContext.setChangedSourceProduct(string);
		System.out.println("Changed Source Product: " + scenarioContext.getChangedSourceProduct());
		processingOrdersPage.selectChangedPrimaryProductFromGrid(scenarioContext.getChangedSourceProduct());
	}

	@Then("search the changed source product and UOM for the variant used in the transformation")
	public void search_the_changed_source_product_and_uom_for_the_variant_used_in_the_transformation() {
		OrderData orderData = scenarioContext.getOrderData();

		stockSummaryPage.clickProductNameFilterIcon();
		stockSummaryPage.enterProductNameInFilterInput(scenarioContext.getChangedSourceProduct());
		processingOrdersPage.clickApplyButton();
		...
		processingOrdersPage.pressEnterKey();
		stockSummaryPage.selectproduct(scenarioContext.getChangedSourceProduct());

	}

	@Then("order status should be {string}")
	public void order_status_should_be(String string) {
//		processingOrdersPage.clickFilterForProcessingOrderId();
//		processingOrdersPage.enterProcessingOrderIdInFilter(scenarioContext.getProcessingOrderId());
//		processingOrdersPage.clickApplyButton();
		processingOrdersPage.clickFilterForProcessingOrderId();
		processingOrdersPage.pressEnterKey();
		processingOrdersPage.getOrderStatus();
		Assert.assertEquals(processingOrdersPage.getOrderStatus(), string, "Excepted order status is not matching");
	}

	@Then("user navigates to the Verify and Approve page by selecting the {string} option from the More button")
	public void user_navigates_to_the_verify_and_approve_page_by_selecting_the_option_from_the_more_button(
			String string) {
		processingOrdersPage.clickMoreButton();
		processingOrdersPage.clickVerifyAndApproveOption();
	}

	@When("user approves the order")
	public void user_approves_the_order() {
		processingOrdersPage.clickApproveButton();
		processingOrdersPage.clickConfirmButton();
		processingOrdersPage.clickFilterForProcessingOrderId();
		processingOrdersPage.pressEnterKey();
	...
	@Then("navigate to the process order page")
	public void navigate_to_the_process_order_page() {
		processingOrdersPage.clickMoreButton();
		processingOrdersPage.clickProcessOrderOption();
	}

	@Then("verify that the primery seller dropdown should not contain the secondary seller {string}")
	public void verify_that_the_primery_seller_dropdown_should_not_contain_the_secondary_seller(String string) {
	  		List<String> primarySellers = processingOrdersPage.getPrimarySellerList();
		System.out.println("Primary Sellers: " + primarySellers);
		Assert.assertTrue(primarySellers.contains(string),
				"Secondary seller found in primary seller dropdown: " + string);
	}
